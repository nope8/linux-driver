...
static int dev_open(struct inode *inode, struct file *filp){
	...
	init_timer(&mydevice->s_timer);
	mydevice->s_timer.function = timer_handler;
	mydevice->s_timer.expires = jiffies + HZ;

	add_timer(&mydevice->s_timer); 	//增加定时器
	atomic_set(&mydevice->counter,0); 	//counter清零 
...
void timer_handler(unsigned long arg)
{
	mod_timer(&mydevice->s_timer,jiffies+HZ);
	atomic_inc(&mydevice->counter);

	printk(KERN_NOTICE "current jiffies:%ld\n",jiffies);
}

/*文件释放函数*/
static int dev_release(struct inode *inode, struct file *filp){
	dev_fasync(-1,filp,0);
	del_timer(&mydevice->s_timer); 	//删除定时器
...
#endif
	counter = atomic_read(&mydevice->counter);
...
/*
void dev_do_work(unsigned long arg)
{
	printk(KERN_INFO "dev_do_work-jiffies:%ld\n",jiffies);
}
*/
...
//	INIT_WORK(&mydevice->my_wq,(void *)dev_do_work);  //for kernel 2.6.20以上
