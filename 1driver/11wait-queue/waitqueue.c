...
static int dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	...
	while(dev->len == 0)
	{
		...
		if(filp->f_flags & O_NONBLOCK) 		//判断进程是否为非阻塞I/O
			return -EAGAIN; 	//err:Try again
		//使进程进入睡眠(条件不成里);否则(条件成立),进程立刻返回
		if(wait_event_interruptible(dev->rq, (dev->len != 0) )) 	
			return -ERESTARTSYS;
		printk("I am waiting for input!!\n");
		down(&dev->sem);
	}

	if(count > dev->len) 	count=dev->len;

	/*完成内核空间data到用户空间buf的copy*/
	if(copy_to_user(buf,dev->data,count)){
		...
		dev->data[dev->len] = '\0';
		printk(KERN_INFO "read from kernel %d bytes,cur_len:%d,cur_string:%s\n\n",count,dev->len,dev->data);
		ret = count;
	}
...
static int dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	...
	else
	{
		dev->len += count;
		dev->data[dev->len] = '\0';
		printk(KERN_INFO "receiving from user %d bytes,current len:%d,current string:%s \n\n",count,dev->len,dev->data);
		ret = count;
	}
