 ...
 * 由于workqueue是在下一次任务切换之前执行，1个Tick之内，所以两个jiffies应该相等。
 * author：lewis
 * date:2013-7-5
...
static int dev_open(struct inode *inode, struct file *filp){
	...
	init_timer(&mydevice->m_timer);
	mydevice->m_timer.function = timer_handler;
	mydevice->m_timer.expires = jiffies + HZ;
	add_timer(&mydevice->m_timer);

	atomic_set(&mydevice->counter,0);
...
void timer_handler(unsigned long arg)
{
	mod_timer(&mydevice->m_timer,jiffies+HZ);
